...
exports.InsertWithPk = (req, res, next) => {
    ...
    if (("KeshavSoft" in req) === false) {
        return res.json({ KTF: false, KReason: "KeshavSoft not found in Request" })
    };
    if (("DataPk" in req.KeshavSoft) === false) {
        return res.json({ KTF: false, KReason: "DataPk not found in KeshavSoft" })
    };

    if (("FolderName" in req.body) === false) {
        return res.json({ KTF: false, KReason: "FolderName not found in body" })
    };

    if (("FileNameOnly" in req.body) === false) {

        return res.json({ KTF: false, KReason: "FileNameOnly not found in body" });
    };

    if (("ItemName" in req.body) === false) {
        return res.json({ KTF: false, KReason: "ItemName not found in body" })
