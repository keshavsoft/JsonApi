exports.PostFunc = (req, res, next) => {
    ...
    if (("MapString" in req.body) === false) {
        res.json({ KTF: false, KReason: "MapString not found in body" })
        return;
    };

    if (("FilterString" in req.body) === false) {
        res.json({ KTF: false, KReason: "FilterString not found in body" })
        return;
    };
    next();
};